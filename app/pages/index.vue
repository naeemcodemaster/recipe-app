<template lang="">
  <main>
    ...
    <section class="py-20 container">
      ...
      <div v-if="!error" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-4">
        <div v-for="recipe in data?.recipes" class="flex flex-col shadow rounded-md">
          <NuxtImg
            ...
            class="rounded-t-md"
          />
          <div class="flex flex-col py-4 px-4 flex-1">
            <p class="text-xl lg:text-xl font-semibold mb-2">{{ recipe.name }}</p>
            ...
            <NuxtLink
              ...
              class="px-4 py-2 text-white self-start bg-primaryColor rounded-md text-base lg:text-lg cursor-pointer hover:bg-secondaryColor"
            >
              View
            </NuxtLink>
          </div>
...
// const {data,error} = await useAsyncData('receipes',()=>$fetch('https://dummyjson.com/recipes?limit=12'));
